...
The mean number of steps per day was `r as.character(round(stepsMean,2))` 
and the median number of steps per day was 
`r as.character(round(stepsMedian,2))`.
...
`r substr(maxInt,12,16)` *[substr(maxInt,12,16)]*
...
`r as.character(round(stepsMean2,2))` steps and the median is 
`r as.character(round(stepsMedian2,2))` steps.  
